...
const Home: React.FC<HomeProps> = ({ onLogout, onNavigate }) => {
  ...
  const handleServiceClick = (nomeServico: string) => {
    const standardizedService = nomeServico.toLowerCase();

    if (standardizedService.includes("exame")) {
      if (onNavigate) {
        onNavigate("exames");
      } else {
        window.location.href = "/exames";
      }
    } else if (standardizedService.includes("consulta")) {
      if (onNavigate) {
  ...
  const handleHeaderClick = () => {
    ...
    // Usar onNavigate se disponível
    if (onNavigate) {
      onNavigate("perfil");
    } else {
      window.location.href = "/perfil-clientes";
    }
  };
  ...
  return (
    <div className={style.body}>
      ...
      <main className={style.main}>
                ...
                <div
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ") {
                      e.preventDefault();
                      handleServiceClick(item.value);
                    }
                  }}
                >
                  <div className={style.iconService}>
        ...
        <section className={style.differencesSection}>
          <div className={style.differencesHeader}>
            <h2>Por que nos escolher?</h2>
            <p>Comprometidos com a sua saúde e bem-estar</p>
          </div>
          <div className={style.differencesGrid}>
            <div className={style.difference}>
              <div className={style.differenceIcon}></div>
