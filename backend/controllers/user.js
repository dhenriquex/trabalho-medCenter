...
`;
    ...
    db.query(queryMedico, [medicoId], (err, dadosMedico) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ error: "Erro ao buscar dados do médico" });
      }
      ...
      db.query(
        queryEstatisticas,
        [medicoId, medicoId, medicoId, medicoId],
        (err, estatisticas) => {
          if (err) {
            console.error(" Erro ao buscar estatísticas:", err);
            return res
            ...
            db.query(
              ...
              (err, consultasSemana) => {
                if (err) {
                  ...
                  return res
                    .status(500)
                    .json({ error: "Erro ao buscar consultas da semana" });
                }
                ...
                db.query(
                  ...
                  (err, examesPendentes) => {
                    if (err) {
                      ...
                      return res
                        .status(500)
                        .json({ error: "Erro ao buscar exames" });
                    }
...
export const atualizarStatusConsulta = (req, res) => {
  const { id } = req.params;
  const { status, observacoes } = req.body;
  if (!status) {
    return res.status(400).json({ error: "Status é obrigatório" });
  }
  const queryVerificar = "SELECT * FROM consulta WHERE id = ?";
  db.query(queryVerificar, [id], (err, results) => {
    if (err) {
...
export const atualizarStatusExame = (req, res) => {
  const { id } = req.params;
  const { status, observacoes } = req.body;
  ...
  const queryVerificar = "SELECT * FROM exame WHERE id = ?";
